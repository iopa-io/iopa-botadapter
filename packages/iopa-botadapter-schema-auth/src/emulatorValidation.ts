...
export async function authenticateEmulatorToken(
    ...
    try {
        ...
        // Async validation.
        const versionClaim: string = identity.getClaimValue(
        ...
        if (!versionClaim || versionClaim === '1.0') {
            ...
            const appIdClaim: string = identity.getClaimValue(
                AuthenticationConstants.AppIdClaim
            )
            if (!appIdClaim) {
            ...
            const appZClaim: string = identity.getClaimValue(
