...
export class JwtTokenExtractor {
    ...
    private static openIdMetadataCache: Map<string, OpenIdMetadata> = new Map<
        string,
        OpenIdMetadata
    >()
    ...
    private static getOrAddOpenIdMetadata(metadataUrl: string): OpenIdMetadata {
        let metadata: OpenIdMetadata = JwtTokenExtractor.openIdMetadataCache.get(
            metadataUrl
        )
        if (!metadata) {
    ...
    private async validateToken(
        ...
        try {
            ...
            if (Array.isArray(endorsements) && endorsements.length !== 0) {
                const isEndorsed: boolean = EndorsementsValidator.validate(
                    channelId,
                    endorsements
                )
                if (!isEndorsed) {
