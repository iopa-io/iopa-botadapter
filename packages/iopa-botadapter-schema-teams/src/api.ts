 ...
 * @enum {string}
 */
export type ApplicationIdentityType =
    | 'aadApplication'
    | 'bot'
    | 'tenantBot'
    | 'office365Connector'
...
export class TeamsApi extends BaseAPI implements TeamsApiInterface {
    ...
    teamsFetchChannelList(
        ...
        return TeamsApiFp(this.configuration).fetchChannelList(teamId, options)(
            this.fetch,
            this.basePath
        )
    }
    ...
    teamsFetchTeamDetails(teamId: string, options?: any): Promise<TeamDetails> {
        return TeamsApiFp(this.configuration).fetchTeamDetails(teamId, options)(
            this.fetch,
            this.basePath
        )
    }
