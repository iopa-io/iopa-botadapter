...
export class TokenHelpers {
    ...
    async getUserToken(connectionName, magicCode) {
        const activity = this.context["bot.Capability"].activity;
        ...
        const url = this.oauthApiUrl(this.context);
        const client = this.createUserTokenApiClient(url);
        return await client.userTokenGetToken(userId, connectionName, activity.channelId, magicCode);
    }
    ...
    async getSignInLink(connectionName) {
        const { adapter, activity } = this.context["bot.Capability"];
    ...
    async getTokenStatus(userId, includeFilter) {
        ...
        const url = this.oauthApiUrl(this.context);
        const client = this.createUserTokenApiClient(url);
        return await client.userTokenGetTokenStatus(userId, activity.channelId, includeFilter);
    }
    /** An asynchronous method that signs out the user from the token server. */
    async getAadTokens(connectionName, resourceUrls) {
        ...
        const url = this.oauthApiUrl(this.context);
        const client = this.createUserTokenApiClient(url);
        return await client.userTokenGetAadTokens(userId, connectionName, { resourceUrls: resourceUrls }, activity.channelId);
    }
    /** Creates an OAuth API client. */
    createUserTokenApiClient(serviceUrl) {
        const fetchProxy = async (url, init) => {
            await this.context["bot.Capability"].adapter.credentials.signRequest(url, init);
